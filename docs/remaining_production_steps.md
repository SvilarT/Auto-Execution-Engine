...
| **12** | **Integrate a real broker adapter behind the existing boundary** | Completed in commit `5b4d834`, which added an Alpaca-backed broker submitter, mode-aware credential/config loading, ambiguous-outcome recovery via client-order-id lookup, and targeted startup/adapter regression coverage | Real adapter implementation, credential/config plumbing, sandbox/paper verification flows, and end-to-end tests |
| **13** | **Add broker-state synchronization for order status and fills** | Now that order submission can reach a real broker, the next production gap is deterministic ingestion of broker truth so fills, cancels, and status changes are reflected without manual intervention | Polling or streaming adapters, durable broker-state snapshots, fill/status reconciliation, and restart-safe recovery tests |
...
The next highest-value build target is **Step 13: add broker-state synchronization for order status and fills**. With a real broker adapter now wired behind the submission boundary, the largest remaining production gap is continuous ingestion of broker truth after submission. The next milestone should capture order-status transitions, fills, cancels, and other broker-side state changes through a deterministic polling or streaming path, persist that truth durably, and prove restart-safe recovery so reconciliation and risk controls operate on up-to-date external state.
